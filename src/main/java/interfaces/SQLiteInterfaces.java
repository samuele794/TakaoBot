		...
		try {
			...
			if (!resultSet.next()) {
				sql = "CREATE TABLE " + SERVERS_DISCORD +
						...
						SD_DB_COLUMN.BDOBossIDChannel + " VARCHAR(255))";
...
//                CREATE TABLE RSSLink ( LastNewsBDO TEXT default NULL, LastPatchBDO TEXT default null, ID INTEGER constraint RSSLink_pk primary key autoincrement )
	...
	public static void setBDOBossChannel(String serverID, String channelID) {
		String sql = "UPDATE " + SERVERS_DISCORD + " SET " + SD_DB_COLUMN.BDOBossIDChannel.name() + "= '" + channelID + "' WHERE " + SD_DB_COLUMN.SERVER_ID.name() + "='" + serverID + "';";
	...
	public static void removeBDOBossChannel(String serverID) {
		String sql = "UPDATE " + SERVERS_DISCORD + " SET " + SD_DB_COLUMN.BDOBossIDChannel.name() + "= NULL WHERE " + SD_DB_COLUMN.SERVER_ID.name() + "='" + serverID + "';";
	...
	public static ArrayList<ServerToChannel> getBDOBossChannel() {
		String sql = "SELECT " + SD_DB_COLUMN.SERVER_ID.name() + ", " + SD_DB_COLUMN.BDOBossIDChannel.name() + " FROM " + SERVERS_DISCORD + " WHERE " + SD_DB_COLUMN.BDOBossIDChannel.name() + " IS NOT NULL";
		ArrayList<ServerToChannel> list = new ArrayList<>();
		...
		try {
			...
			while (resutlt.next()) {
				list.add(new ServerToChannel(resutlt.getString(SD_DB_COLUMN.SERVER_ID.name()), resutlt.getString(SD_DB_COLUMN.BDOBossIDChannel.name())));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
